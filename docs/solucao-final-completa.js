/**
 * Solução FINAL COMPLETA para GitHub Pages
 * Versão 100% funcional que corrige todos os problemas conhecidos
 */
(function() {
    console.log('[FinalCompleta] Inicializando...');
    ...
    let rotaOriginalHtml = ''; // Para armazenar o conteúdo original da sequência de rota
    ...
    function inicializar() {
        if (window.inicializacaoCompletaFinal) {
            return;
        }
        ...
        if (!window.google || !window.google.maps) {
            console.log('[FinalCompleta] Aguardando carregamento do Google Maps API...');
            setTimeout(inicializar, 1000);
            return;
        }
        ...
        // Configurar o botão de otimizar
        configurarBotaoOtimizar();
        
        // Monitorar os containers que aparecem após a otimização
        monitorarContainers();
        
        // Marcar como inicializado
        window.inicializacaoCompletaFinal = true;
        console.log('[FinalCompleta] Inicialização completa');
    }
    ...
    function initServicos() {
        console.log('[FinalCompleta] Inicializando serviços do Google Maps');
        ...
        // Procurar na variável global
        if (window.map instanceof google.maps.Map) {
            mapInstance = window.map;
        } else {
            // Procurar outras variáveis que possam ter o mapa
        ...
        // Se não encontrou, verificar se existe elemento para criar
        if (!mapInstance) {
            ...
            if (mapElement) {
                console.log('[FinalCompleta] Criando novo mapa');
        ...
        // Configurar renderer com o mapa (se encontrado)
        if (mapInstance) {
            try {
                directionsRenderer.setMap(mapInstance);
            } catch (e) {
                console.error('[FinalCompleta] Erro ao configurar renderer:', e);
            }
        }
    }
    ...
    function adicionarEstilos() {
        if (document.getElementById('final-completa-styles')) {
            return;
        }
        
        console.log('[FinalCompleta] Adicionando estilos CSS');
        
        const style = document.createElement('style');
        style.id = 'final-completa-styles';
        style.textContent = `
            ...
            .sequencia-personalizada .sequence-item {
                margin-bottom: 5px;
    ...
    function configurarArrastar() {
        console.log('[FinalCompleta] Configurando arrastar e soltar');
        ...
        if (!container) {
            console.log('[FinalCompleta] Container de destinos não encontrado, tentando novamente');
        ...
        console.log('[FinalCompleta] Configurando', itens.length, 'itens existentes');
                    ...
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === 1) {
                            if (!node.classList.contains('origin-point') &&
                                ...
                                console.log('[FinalCompleta] Novo item configurado como arrastável');
                            }
                        }
                    });
                }
    ...
    function salvarDestinosAtuais() {
        ...
        if (!container) {
            console.error('[FinalCompleta] Container de destinos não encontrado');
            return;
        }
        ...
        console.log('[FinalCompleta] Encontrados', itens.length, 'destinos');
        ...
        itens.forEach((item, index) => {
            ...
            destinos.push({
                ...
                lat: coordenadas.lat,
                lng: coordenadas.lng,
                ordem: index + 1 // A ordem sequencial (origem é 0)
            });
            
            console.log('[FinalCompleta] Destino salvo:', nome, coordenadas);
        });
    }
    ...
    function configurarBotaoOtimizar() {
        ...
        if (!botao) {
            console.log('[FinalCompleta] Botão de otimizar não encontrado, tentando novamente');
            setTimeout(configurarBotaoOtimizar, 1000);
            return;
        }
        
        console.log('[FinalCompleta] Configurando botão de otimizar');
        ...
        botao.onclick = function(e) {
            e.preventDefault();
            console.log('[FinalCompleta] Botão de otimizar clicado');
            ...
            // Salvar a ordem original antes da otimização
            setTimeout(salvarSequenciaOriginal, 100);
    ...
    /**
     * Salvar sequência original da rota
     */
    function salvarSequenciaOriginal() {
        // Encontrar a sequência de rota
        const sequencia = document.getElementById('route-sequence');
        
        if (sequencia) {
            // Armazenar o HTML original
            rotaOriginalHtml = sequencia.innerHTML;
            console.log('[FinalCompleta] Sequência original da rota salva');
        }
    }
    
    /**
     * Monitorar containers diversos
     */
    function monitorarContainers() {
        console.log('[FinalCompleta] Configurando monitoramento de containers');
        
        // Verificar periodicamente por rotas alternativas
        setInterval(() => {
            // Verificar container de rotas alternativas
            const containerAlternativas = document.querySelector('.alternative-routes-section') || 
                                        document.querySelector('[class*="alternative-routes"]');
            
            if (containerAlternativas) {
                // Verificar se já existe nossa opção
                const existente = containerAlternativas.querySelector('.rota-personalizada-card');
                
                if (!existente) {
                    console.log('[FinalCompleta] Container de alternativas encontrado sem nossa opção');
                    adicionarOpcaoRotaPersonalizada();
                }
            }
            
            // Monitorar também o relatório de rota
            monitorarRelatorioRota();
        }, 1000);
    }
     ...
     */
    function monitorarRelatorioRota() {
        ...
        if (relatorio && relatorio.style.display !== 'none') {
            console.log('[FinalCompleta] Relatório de rota detectado');
            
            // Se a rota personalizada está ativa, atualizar a sequência
            if (rotaPersonalizadaAtiva) {
                atualizarSequenciaNoRelatorio();
            }
        }
    }
    
    /**
     * Atualizar a sequência de visitas no relatório
     */
    function atualizarSequenciaNoRelatorio() {
        // Encontrar a sequência no relatório
        const sequencia = document.querySelector('.route-sequence') || 
                         document.querySelector('.sequencia-container') ||
                         document.querySelector('[class*="sequencia"]');
        
        if (!sequencia) {
            return;
        }
        
        console.log('[FinalCompleta] Atualizando sequência no relatório');
    ...
    function adicionarOpcaoRotaPersonalizada() {
        console.log('[FinalCompleta] Adicionando opção de rota personalizada');
        ...
        if (!container) {
            console.log('[FinalCompleta] Container de rotas alternativas não encontrado');
            return;
        }
        
        // Verificar se já existe nossa opção
        if (container.querySelector('.rota-personalizada-card')) {
            console.log('[FinalCompleta] Opção de rota personalizada já existe');
            return;
        }
        ...
        if (destinos.length === 0) {
            ...
            if (destinos.length === 0) {
                console.log('[FinalCompleta] Sem destinos para adicionar rota personalizada');
                return;
            }
        }
        ...
        card.addEventListener('click', function() {
            console.log('[FinalCompleta] Card de rota personalizada clicado');
        ...
        console.log('[FinalCompleta] Opção de rota personalizada adicionada');
    }
    ...
    function calcularRotaPersonalizada() {
        console.log('[FinalCompleta] Calculando rota personalizada');
        ...
        if (destinos.length === 0) {
            ...
            if (destinos.length === 0) {
                console.error('[FinalCompleta] Sem destinos para calcular rota');
                alert('É necessário adicionar pelo menos um destino para calcular a rota.');
                return;
            }
        }
        ...
        if (!mapInstance) {
            ...
            if (mapElement) {
                ...
                console.error('[FinalCompleta] Elemento do mapa não encontrado');
        ...
        console.log('[FinalCompleta] Enviando solicitação de direções');
        
        // Calcular rota
        directionsService.route(request, function(result, status) {
            if (status === google.maps.DirectionsStatus.OK) {
                console.log('[FinalCompleta] Rota calculada com sucesso');
                ...
                // Atualizar também a sequência na interface
                atualizarSequenciaInterfacePersonalizada([origem, ...destinos]);
            } else {
                console.error('[FinalCompleta] Erro ao calcular rota:', status);
    ...
    function atualizarInterfaceComRotaCalculada(result, pontos) {
        console.log('[FinalCompleta] Atualizando interface com resultados');
        ...
        console.log('[FinalCompleta] Distância total:', distanciaKm, 'km');
        console.log('[FinalCompleta] Tempo total:', horas + 'h', minutos + 'min');
    ...
    /**
     * Atualizar a sequência na interface
     */
    function atualizarSequenciaInterfacePersonalizada(pontos) {
        // Encontrar container de sequência
        const routeSequence = document.getElementById('route-sequence');
        
        if (!routeSequence) {
            console.log('[FinalCompleta] Container de sequência não encontrado');
            return;
        }
        
        console.log('[FinalCompleta] Atualizando sequência na interface');
        
        // Criar HTML da sequência
        let html = '<div class="mt-3 mb-2"><strong>Sequência de Visitas (Personalizada):</strong></div>';
        html += '<div class="sequencia-personalizada">';
    ...
    function desenharRotaManual(mapa, pontos) {
        console.log('[FinalCompleta] Desenhando rota manualmente como fallback');
        ...
        // Atualizar sequência na interface
        atualizarSequenciaInterfacePersonalizada(pontos);
    }
