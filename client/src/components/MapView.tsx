...
export default function MapView({ 
  ...
  // Quando waypoints mudam, atualizar o mapa e marcadores completamente
  useEffect(() => {
    if (origin) {
      // Forçar uma recriação do mapa se os waypoints mudaram
      if (waypoints.length > 0) {
        console.log("Waypoints mudaram, recriando mapa e marcadores...");
        
        // Limpar mapa anterior e forçar recriação
        if (directMapRef.current) {
          // Remover todos os marcadores existentes antes de recriar
          if (markersRef.current.length > 0) {
            console.log(`Limpando ${markersRef.current.length} marcadores existentes para recriação`);
          ...
          try {
            // Inicializar mapa novamente com as configurações corretas
            // Criar nova instância do mapa com as mesmas configurações
            const originCoords = { 
              lat: parseFloat(origin.lat), 
              lng: parseFloat(origin.lng) 
            };
            ...
            console.log("Mapa e marcadores atualizados com sucesso após mudança de waypoints");
          } catch (err) {
            console.error("Erro ao recriar mapa e marcadores:", err);
          }
